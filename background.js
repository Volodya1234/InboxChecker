...
function getXMLNode( node, tagName )
{
	var value = node.getElementsByTagName( tagName );
...
function startRequest( doNotShowNotification )
{
	...
	try
	{
		xhr.onreadystatechange = function()
		{
			...
			updateIcon( getXMLNode( xhr.responseXML, 'fullcount' ) );
			
			chrome.storage.local.get( 'lastMessage', function( item )
			{
				var currentLastMessage = item.lastMessage || 0;
				var lastMessage = 0;
				...
				{
					var entry = entries[ i ];
					var date = new Date( getXMLNode( entry, 'modified' ) ).getTime();
					
					if( currentLastMessage >= date )
					{
						return;
					}
				...
				if( lastMessage > 0 )
				{
					chrome.storage.local.set( { lastMessage: lastMessage } );
				}
			} );
		};
...
chrome.notifications.onClicked.addListener( openNotification );

startRequest();

chrome.alarms.onAlarm.addListener( startRequest );
chrome.alarms.create( 'InboxPoll', { periodInMinutes: 1 } );
